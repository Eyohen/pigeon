...
const SuperAdmin = () => {
  ...
  const handleSubmit = async() => {
    ...
    setIsLoading(true)
    try{
      const res = await axios.post(URL+"/api/auth/superadminlogin", {email,password})
  ...
  return (
    ...
    <div className='flex justify-center '>
        ...
        <p className='text-center font-semibold text-3xl mt-3'>Login To Your Super Admin Account</p>
  ...
  <div className='flex justify-between'>
    ...
    <Link to={'/forgotpassword'}><p className='text-[#F08E1F]'>Forgot Password</p></Link>
    </div>
        ...
        <button onClick={handleSubmit}  className='bg-[#F7F7F7] text-[#98999A] w-full md:w-[450px] py-2 rounded-2xl mt-6 hover:bg-[#F08E1F] hover:text-white'>{isLoading ? "Loading..." : "Login"}</button>
        {error && <h3 className='text-red-500 text-lg text-center'>Something went wrong</h3>}
        </div>
        <p className='pt-3 text-center text-[#98999A]'>Don't have an account?   <Link to={'/register'}><span className='text-[#F08E1F] ml-3'>Create an account</span></Link></p>
        </div>
